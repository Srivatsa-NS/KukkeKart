...
function ProductModal({ isOpen, onClose, product, bgColor, onUpdate }: ProductModalProps) {
  const getInitialQuantities = () => {
    const cartItems = cartStore.getItems().filter(item => item.name === product.name);
    const quantities: Record<string, number> = { "250g": 0, "500g": 0, "1kg": 0 };
  ...
  const [quantities, setQuantities] = useState<Record<string, number>>(getInitialQuantities());
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-[70%] w-full p-6 md:p-8">
            <div className="flex justify-between items-start mb-6">
              <h2 className={`headings-font text-3xl md:text-5xl text-${bgColor}`}>
                {product.name}
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="mb-8">
              <h3 className={`body-font text-xl md:text-2xl text-${bgColor} mb-4`}>
                Select Quantity
              </h3>
              <div className="space-y-4">
                {weightOptions.map((option) => (
                  <div key={option.value} className={`flex items-center justify-between p-4 rounded-xl border-2 border-${bgColor}`}>
                    <div className="flex items-center gap-4">
                      <span className={`body-font text-lg md:text-xl font-semibold text-${bgColor}`}>
                        {option.label}
                      </span>
                      <span className={`body-font text-sm md:text-base text-${bgColor}`}>
                        ₹{getPrice(option.value)}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => handleQuantityChange(option.value, -1)}
                      ...
                      <span className={`body-font text-lg md:text-xl font-bold text-${bgColor} min-w-[2rem] text-center`}>
            ...
            <div className={`mb-6 p-4 rounded-xl bg-${bgColor} bg-opacity-10`}>
              <div className="flex justify-between items-center">
                <span className={`body-font text-lg md:text-xl text-${oppositeColor}`}>
                  Total Price:
                </span>
                <span className={`body-font text-2xl md:text-3xl font-bold text-${oppositeColor}`}>
                  ₹{getTotalPrice()}
                </span>
              </div>
            </div>
